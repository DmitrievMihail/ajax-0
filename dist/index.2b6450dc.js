...
function createOptionsSelector(resp) {
    ...
    if (!cities.length) throw new Error(); // Загрузили какую-то фигню а не города, вываливаемся в errorLoadingCities
...
fetch("https://city-list.p.rapidapi.com/api/getCity/ru", APIoptions).then((response)=>response.json()).then((response)=>createOptionsSelector(response[0])).catch(()=>errorLoadingCities());
function errorLoadingCities() {
    // TODO убрать innerHTML
    form.innerHTML = `<h1>Произошла ошибка</h1><b>Ошибка загрузки городов</b>.<br>` + `<br><span style='color:red;'>Прогноз погоды предоставить невозможно</span><br><br><b><a href="#">Обновите страницу</b>`;
    throw new Error("Ошибка загрузки городов");
}
function generateCityList(response) {
    for(const key in response){
        if (response[key].level.slice(0, 3) === "ADM" || response[key].population < 10000) continue;
        ...
        for (const cityName of JSON.parse(response[key].alternames)){
            let rusLng = true;
            if (cityName) {
                for (const char of cityName)if (!rusLetters.has(char)) {
                    rusLng = false;
                    continue;
                }
            }
...
function errorLoadingWether() {
    ...
    cityHTML.disabled = false;
    submit.disabled = false;
    outputHTML.innerHTML = '<b style="color:red;">Ошибка загрузки погоды.</b><br><br> Попробуйте позднее или выберите другой город';
...
async function loadWether(cityIndex) {
    ...
    const response = await fetch(`https://weatherbit-v1-mashape.p.rapidapi.com/forecast/3hourly?lat=${cities[cityIndex].lat}&lon=${cities[cityIndex].long}&lang=ru`, APIoptions).then((response)=>response.json()).catch(()=>errorLoadingWether());
...
form.addEventListener("submit", async (event)=>{
    ...
    if (cityIndex !== -1) {
        ...
        if (!weather[cityIndex] || !weather[cityIndex].data) {
            ...
            for(const field in fields)html += `\n<th>${fields[field]}</th>`;
            html += "</tr>";
            for (const currentWeather of weather[cityIndex].data){
                html += '\n<tr style="text-align:center;">';
                for(const field1 in fields)if (field1.indexOf("*") === -1) html += `\n<td>${currentWeather[field1]}</td>`;
                else {
                    const subfields = field1.split("*");
...
//# sourceMappingURL=index.2b6450dc.js.map
